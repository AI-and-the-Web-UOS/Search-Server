...
index = IndexUpdater(database_name)
...
def add_view():
    ...
    database.insert_or_update_view(url, current_week, current_year)

    return "", 200

if __name__ == '__main__':
    # Start a thread for the relevance periodic task
    relevance_thread = threading.Thread(target=periodic_task, args=(database_name,), daemon=True)
